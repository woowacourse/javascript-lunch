...
- [ ] `templates` 폴더에 있는 코드를 프로덕트에 적용시킨다.
- [ ] 프로덕트 코드가 제대로 적용되어 있는지 확인한다.
  - [ ] (test) 프로덕트 코드 동작 -> body나 최상단 태그가 렌더링 되었는지 확인한다.
---
### 메인
- [ ] `header` (.gnb)
  - [ ] (템플릿 코드 기반) `header` 태그를 바탕으로 테스트 작성
  - [ ] (프로덕트 코드) `header` 태그 삭제 -> 테스트 실패
  - [ ] (테스트 실패를 성공시키기 위해) js를 이용해 동적으로 `header` 태그를 제작 -> 테스트 성공
  - [ ] `header` 태그를 컴포넌트로 제작 
  - [ ] `header` 컴포넌트 리팩토링 (동적 인자 전달/컴포넌트 이동 및 분리)
    - [ ] `header` 테스트 코드 수정

- [ ] `list-item` (.restaurant)  (menu-item으로 안한 이유 : 확정성 고려)
  - [ ] (템플릿 코드 기반) `li`(.restaurant) 태그를 바탕으로 테스트 작성
  - [ ] (프로덕트 코드) `li` 태그 전부 삭제 -> 테스트 실패
  - [ ] (테스트 실패를 성공시키기 위해) js를 이용해 동적으로 `li` 태그 하나만 제작 -> 테스트 성공
  - [ ] `li` 태그를 컴포넌트로 제작 
  - [ ] `li` 컴포넌트 리팩토링 (동적 인자 전달/컴포넌트 이동 및 분리)
    - [ ] 동적 인자 전달 -> 정적 텍스트를 어떻게 관리할거냐 -> 입력한 값도 추가가 되어야하고, 기존에 있는 값도 추가가 되어 있어야한다. -> 도메인 (식당 정보들을 관리하고 있는 것! -> 인스턴스)
    - [ ] `li` 테스트 코드 수정


- [ ] `list` (.restaurant-list) 
  - [ ] (템플릿 코드 기반) `ul`(.restaurant-list) 태그를 바탕으로 테스트 작성
  - [ ] (프로덕트 코드) `ul` 태그 전부 삭제 -> 테스트 실패
  - [ ] (테스트 실패를 성공시키기 위해) js를 이용해 동적으로 `ul` 태그 제작 -> 테스트 성공
  - [ ] `ul` 태그를 컴포넌트로 제작 
  - [ ] `ul` 컴포넌트 리팩토링 (동적 인자 전달/컴포넌트 이동 및 분리)
    - [ ] 동적 인자 -> `list-item` 컴포넌트
    - [ ] `ul` 테스트 코드 수정

### 모달

- [ ] `select-form` (label + select.category + notice(옵션) -> 묶고 있는 div) 
  - [ ] (템플릿 코드 기반) `select-form` 태그를 바탕으로 테스트 작성
  - [ ] (템플릿 코드 기반) `select`, `label`, `notice` 태그가 있는지 테스트 작성
  - [ ] (프로덕트 코드) `select-form` 태그 삭제 -> 테스트 실패
  - [ ] (테스트 실패를 성공시키기 위해) js를 이용해 동적으로 `select-form` 태그 제작 -> 테스트 성공
  - [ ] `select-form` 태그를 컴포넌트로 제작 
