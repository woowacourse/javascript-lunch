...
describe('음식점 업데이트 테스트 서비스', () => {
  ...
  // beforeEach(() => {
  //   cy.stub(localStorage, 'getItem', () => localStorage.getItem(RESTAURANTS_DB_TEST_KEY));
  //   cy.stub(localStorage, 'setItem', (restaurantJSON) => {
  //     // JSON.parse(restaurantJSON);
  //     localStorage.setItem(RESTAURANTS_DB_TEST_KEY, restaurantJSON);
  //   });
  ...
  // it('음식점을 삭제했을 떄, DB서비스에서 업데이트한 데이터에 반영이 된다.', () => {
  //   const NEW_RESTAURANT1: IRestaurant = {
  //     id: 1,
  //     name: '리안네 볶음밥',
  //     distance: 10,
  ...
  //   const restaurantUpdateService = new RestaurantUpdateService();
  //   const restaurantDBService = new RestaurantDBService();
  //   restaurantDBService.set(new RestaurantCollection([NEW_RESTAURANT1, NEW_RESTAURANT2]));

  //   restaurantUpdateService.deleteRestaurant(2);

  //   expect(restaurantDBService.update().get().length).to.equal(1);
  //   expect(restaurantDBService.update().get()).to.deep.equal([NEW_RESTAURANT1]);
  // });
