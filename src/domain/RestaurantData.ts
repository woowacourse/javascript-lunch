...
export default class RestaurantData {
  ...
  private validateCategory(category: CategoryType): void {
    if (!category) throw Error(RestaurantData.ERROR_MESSAGE.category);
  }

  private validateDistance(distance: number): void {
    if (!distance) throw Error(RestaurantData.ERROR_MESSAGE.distance);
  }

  private validateName(name: string): void {
    if (this.isValidateNameLength(name)) {
      throw Error(RestaurantData.ERROR_MESSAGE.nameLength);
    }
    if (this.hasInvalidNameChar(name)) {
      throw Error(RestaurantData.ERROR_MESSAGE.nameChar);
    }
  }

  private validateLink(link: string): void {
    if (this.isValidateLink(link)) {
      throw Error(RestaurantData.ERROR_MESSAGE.link);
    }
  }
}
